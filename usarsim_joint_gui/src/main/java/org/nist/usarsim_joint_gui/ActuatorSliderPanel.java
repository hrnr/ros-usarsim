...
public class ActuatorSliderPanel extends SliderPanel {
    ...
    public ActuatorSliderPanel(String confStr, int percision, Node node) {
        super();
	myNode = node;
    ...
    private ArrayList<ActuatorControl> getControllers(){
        ...
        while (scan.hasNext()){
			...
			ActuatorControl ac = new ActuatorControl(temp.substring(0, temp.length()-1), myNode);
                actuators.add(ac);
            }
	...
	*/
	...
	private control_msgs.FollowJointTrajectoryActionGoal goal;
	    ...
	    System.out.println( "Slider for actuator " + name + " changed to " + (((double)value)/multiplier));
            }
    }
