...
void NavigationGoal::moveOffset(float xGoal, float yGoal, float zGoal)
{
	if(useGlobalPositionFrame)
	{
		...
		try
		{
			listener.waitForTransform("/base_footprint",effectorFrame,ros::Time(0), ros::Duration(10.0));
		...
		ROS_ERROR("global point coords are %f, %f, %f",globalPoint.x(), globalPoint.y(), globalPoint.z());
...
void NavigationGoal::setGlobalPositionGoal(float xGoal, float yGoal, float zGoal)
{
	...
	try
	{
		listener.waitForTransform(effectorFrame, "/base_footprint",ros::Time(0), ros::Duration(10.0));
